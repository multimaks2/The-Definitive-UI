 ...
 *  FILE:        source/PauseMenu/PauseMenu.cpp
...
namespace
{
    const char* ButtonKeys[PauseMenu::Layout::Count] = {
        "UI_CONTINUE",
        "UI_MAP",
        "UI_GAME",
        "UI_STATS",
    ...
    const char* HoverTexNames[PauseMenu::Layout::Count] = {
        PauseMenu::Tex::Start,
        PauseMenu::Tex::Game,
        PauseMenu::Tex::Game,
        PauseMenu::Tex::Settings,
        PauseMenu::Tex::Settings,
        PauseMenu::Tex::Exit
    };
    ...
    const char* StatsCategoryKeys[PauseMenu::Layout::StatsCategoryCount] = {
        "FES_PLA", "FES_MON", "FES_WEA", "FES_CRI",
        "FES_GAN", "FES_ACH", "FES_MIS", "FES_MSC"
    };

    std::string DecodeGxtScratch(uintptr_t addr)
    {
        const char* raw = reinterpret_cast<const char*>(addr);
        if (!raw || !raw[0])
            return {};
        const std::string decoded = SaveSlots::DecodeHudText(raw);
        return decoded.empty() ? std::string(raw) : decoded;
    }

    constexpr uintptr_t kGxtString  = 0xC1B100;
    constexpr uintptr_t kGxtString2 = 0xC1AED8;
}
...
bool PauseMenu::Initialize(LPDIRECT3DDEVICE9 pDevice, Draw* pDraw, TxdManager* pTxd,
...
bool PauseMenu::LoadHoverTextures()
{
...
void PauseMenu::ReleaseMapTextures()
{
...
void PauseMenu::ReleaseFogMaskRT()
{
    if (m_pFogMaskSurf)
    {
...
bool PauseMenu::EnsureFogMaskRT()
{
    if (!m_pDevice)
        return false;
...
void PauseMenu::ReleaseZonesTex()
{
    if (m_pZonesTex)
    {
        m_pZonesTex->Release();
...
void PauseMenu::CenterMapOnPlayer(float screenW, float screenH)
{
...
void PauseMenu::EnterMapFullscreen()
{
    if (m_bMapFullscreen)
        return;
...
void PauseMenu::ExitMapFullscreen()
{
    if (!m_bMapFullscreen)
        return;
    m_bMapFullscreen = false;
...
PauseMenu::MapLayout PauseMenu::ComputeMapLayout(float screenW, float screenH) const
{
    MapLayout out{};
    if (m_bMapFullscreen)
    {
...
void PauseMenu::ClampMapPan(float areaW, float areaH, float mapSize)
{
...
bool PauseMenu::MapScreenToWorld(float screenX, float screenY, const MapLayout& lay, float& outX, float& outY) const
{
    if (lay.mapSize < 1.0f)
        return false;
...
void PauseMenu::ClearMapWaypoint()
{
...
void PauseMenu::PlaceMapWaypoint(float worldX, float worldY)
{
    ClearMapWaypoint();
...
void PauseMenu::ApplyMapZoomFocus(float screenW, float screenH)
{
    if (!m_bMapZoomFocus)
        return;
...
bool PauseMenu::MapTilesReady() const
{
...
void PauseMenu::WarmMapResources()
{
    if (!m_pDevice)
        return;
...
bool PauseMenu::WarmMapTexturesStep(int budget)
{
    if (budget < 1)
        budget = 1;
...
bool PauseMenu::LoadMapTextures()
{
    if (m_bMapTxdReady)
        return true;
...
bool PauseMenu::EnsureStockRadarTextures()
{
...
void PauseMenu::DrawMapPlane(float mapL, float mapT, float mapSize)
{
...
bool PauseMenu::BuildFogContourMask()
{
...
void PauseMenu::DrawMapFogShader(float mapL, float mapT, float mapSize,
...
void PauseMenu::DrawMapExploredOutlines(float mapL, float mapT, float mapSize)
{
...
void PauseMenu::DrawMapTiles(float screenW, float screenH)
{
...
void PauseMenu::DrawMapGps(float screenW, float screenH)
{
...
void PauseMenu::DrawMapLeftArt(float screenW, float screenH)
{
...
void PauseMenu::DrawMapBlips(float screenW, float screenH)
{
...
void PauseMenu::GetMapHoverPlaceNameUtf8(float screenW, float screenH, char* out, size_t outChars) const
{
...
void PauseMenu::DrawMapHintBar(float screenW, float screenH)
{
...
PauseMenu::ButtonRect PauseMenu::GetMapLegendRect(float screenW, float screenH) const
{
    const float sx = screenW / Layout::RefW;
    const float sy = screenH / Layout::RefH;
...
bool PauseMenu::IsCursorOnMapLegend(float cursorX, float cursorY, float screenW, float screenH) const
{
...
void PauseMenu::HandleMapLegendToggle()
{
...
void PauseMenu::DrawMapLegend(float screenW, float screenH)
{
...
void PauseMenu::GetScreenSize(float& outW, float& outH) const
{
    Ui::GetScreenSizeViewport(m_pDevice, outW, outH);
}

bool PauseMenu::GetCursorPosClient(float screenW, float screenH, float& outX, float& outY) const
{
    return Ui::GetCursorPosClient(screenW, screenH, outX, outY);
}

void PauseMenu::ShowOsCursor()
{
...
PauseMenu::ButtonRect PauseMenu::GetButtonRect(int index, float screenW, float screenH) const
{
    const float sx = screenW / Layout::RefW;
    const float sy = screenH / Layout::RefH;
...
PauseMenu::ButtonRect PauseMenu::GetExitConfirmBtnRect(int which, float screenW, float screenH) const
{
    const float sx = screenW / Layout::RefW;
    const float sy = screenH / Layout::RefH;
...
int PauseMenu::HitTestButton(float cursorX, float cursorY, float screenW, float screenH) const
{
    int best = -1;
...
void PauseMenu::SyncActiveFromPanel()
{
    if (!m_pMainMenu)
        return;
    ...
    case MainMenu::Panel::Settings:
        ...
        break;
    default:
        // Game/Settings are owned by MainMenu — if that panel closed, fall back to Map.
        // Stats is local to PauseMenu and must stay active until another tab is picked.
...
void PauseMenu::UpdateHoverSound(float screenW, float screenH)
{
...
void PauseMenu::ConsumeEscJustPressed()
{
    CPad::OldKeyState.esc = CPad::NewKeyState.esc;
}

void PauseMenu::LeavePauseSession()
{
    CloseExitConfirm();
    if (m_pMainMenu)
...
void PauseMenu::OnButtonActivated(int index)
{
    PlayFe(AE_FRONTEND_SELECT);
...
void PauseMenu::UpdateMapZoom(float screenW, float screenH)
{
    if (!IsMapOpen())
        return;
...
void PauseMenu::HandleMapInput(float screenW, float screenH)
{
    if (!IsMapOpen())
    {
        m_bMapPress = false;
...
void PauseMenu::HandleClicks(float screenW, float screenH)
{
    ...
    if (IsStatsOpen())
    {
        const ButtonRect panel = GetContentPanelRect(screenW, screenH);
        if (panel.Contains(cx, cy))
        {
            const float sx = screenW / Layout::RefW;
            const float sy = screenH / Layout::RefH;
            const float rankPadX = Layout::PanelPadX * sx;
            const float rowH = Layout::StatsRowH * sy;
            const float listTop = panel.top;
            const float catArrow = Layout::StatsCatArrowSz * sx;
            const float localY = cy - listTop + m_fStatsScroll - rowH;
            if (localY >= 0.0f && localY < rowH)
            {
                const float catRowTop = listTop + rowH - m_fStatsScroll;
                const ButtonRect catLeft = { panel.left + rankPadX, catRowTop, panel.left + rankPadX + catArrow * 2.0f, catRowTop + rowH };
                const ButtonRect catRight = { panel.right - rankPadX - catArrow * 2.0f, catRowTop, panel.right - rankPadX, catRowTop + rowH };
                if (catLeft.Contains(cx, cy) && m_nStatsCategory > 0)
                {
                    --m_nStatsCategory;
                    m_fStatsScroll = 0.0f;
                    PlayFe(AE_FRONTEND_SELECT);
                }
                else if (catRight.Contains(cx, cy) && m_nStatsCategory + 1 < Layout::StatsCategoryCount)
                {
                    ++m_nStatsCategory;
                    m_fStatsScroll = 0.0f;
                    PlayFe(AE_FRONTEND_SELECT);
...
void PauseMenu::DrawUiText(float left, float top, float right, float bottom, const char* text,
...
void PauseMenu::OpenExitConfirm()
{
    if (m_pMainMenu)
        m_pMainMenu->ClosePanel();
    ResetMapView();
...
void PauseMenu::CloseExitConfirm()
{
    m_bExitConfirm = false;
...
void PauseMenu::DrawExitConfirm(float screenW, float screenH)
{
...
void PauseMenu::ResetStatsView()
{
    m_nStatsCategory = 0;
    m_fStatsScroll = 0.0f;
}

PauseMenu::ButtonRect PauseMenu::GetContentPanelRect(float screenW, float screenH) const
{
    const float sx = screenW / Layout::RefW;
    const float sy = screenH / Layout::RefH;
    return {
        ...
        (Layout::PanelListTopY + Layout::PanelH) * sy
    };
}

void PauseMenu::DrawStatsPanel(float screenW, float screenH)
{
    if (!m_pDraw || !IsStatsOpen())
        return;

    const float sx = screenW / Layout::RefW;
    const float sy = screenH / Layout::RefH;
    const ButtonRect panel = GetContentPanelRect(screenW, screenH);
    const float pw = panel.right - panel.left;
    const float padX = Layout::PanelPadX * sx;
    const float rowH = Layout::StatsRowH * sy;
    const float listTop = panel.top;
    const float listBottom = panel.bottom;
    const float listH = listBottom - listTop;
    const DWORD textCol = 0xFFFFFFFF;

    const int statCount = CStats::ConstructStatLine(99'999, static_cast<unsigned char>(m_nStatsCategory));
    const int totalRows = 2 + statCount;
    const float visibleRows = listH / rowH;
    const float maxScroll = std::max(0.0f, static_cast<float>(totalRows) - visibleRows) * rowH;
    if (m_fStatsScroll < 0.0f)
        m_fStatsScroll = 0.0f;
    if (m_fStatsScroll > maxScroll)
        m_fStatsScroll = maxScroll;

    const std::string rankLabel = DecodeGxtScratch(reinterpret_cast<uintptr_t>(CStats::FindCriminalRatingString()));
    const char* rankPrefix = HelpGxt::Get("CRIMRA");
    if (!rankPrefix)
        rankPrefix = LanguageManager::Get("CRIMRA");
    char rankLine[256];
    std::snprintf(rankLine, sizeof(rankLine), "%s%s (%d)",
                  rankPrefix ? rankPrefix : "", rankLabel.c_str(), CStats::FindCriminalRatingNumber());

    const char* catKey = StatsCategoryKeys[m_nStatsCategory];
    const char* catLabel = HelpGxt::Get(catKey);
    if (!catLabel)
        catLabel = TheText.Get(catKey);
    if (!catLabel)
        catLabel = LanguageManager::Get(catKey);

    float cursorX = 0.0f, cursorY = 0.0f;
    GetCursorPosClient(screenW, screenH, cursorX, cursorY);

    const int rankFontH = static_cast<int>(Layout::StatsRankFont * sy + 0.5f);
    const int rowFontH = static_cast<int>(Layout::StatsFont * sy + 0.5f);
    const float catArrow = Layout::StatsCatArrowSz * sx;

    auto drawPlain = [&](float l, float t, float r, float b, const char* txt, DWORD col, DWORD fmt, int fontH) {
        if (!txt || !txt[0])
            return;
        m_pDraw->EnsureFontHeight(fontH > 0 ? fontH : 1);
        m_pDraw->DrawString(l, t, r, b, col, txt, 1.0f, 1.0f, fmt, false);
    };

    int row = 0;

    // Row 0 — criminal rank (merged with list, no separate header plate).
    {
        const float y = listTop + static_cast<float>(row) * rowH - m_fStatsScroll;
        if (y + rowH >= listTop && y < listBottom)
        {
            const DWORD bg = (row & 1) ? Layout::PanelZebraB : Layout::PanelZebraA;
            m_pDraw->DrawRect(panel.left, y, pw, rowH, bg);
            drawPlain(panel.left + padX, y, panel.right - padX, y + rowH,
                      rankLine, textCol, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, rankFontH);
        }
        ++row;
    }

    // Row 1 — category title inside the list.
    {
        const float y = listTop + static_cast<float>(row) * rowH - m_fStatsScroll;
        if (y + rowH >= listTop && y < listBottom)
        {
            ...
            m_pDraw->DrawRect(panel.left, y, pw, rowH, bg);
            drawPlain(panel.left + padX, y, panel.right - padX, y + rowH,
                      catLabel ? catLabel : "", textCol, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, rowFontH);

            const float catMidY = y + rowH * 0.5f;
            if (cursorY >= y && cursorY < y + rowH)
            {
                if (cursorX < panel.left + padX + catArrow * 2.0f && m_nStatsCategory > 0)
                    m_pDraw->DrawArrowAA(panel.left + padX + catArrow, catMidY, catArrow, true, textCol);
                if (cursorX > panel.right - padX - catArrow * 2.0f
                    && m_nStatsCategory + 1 < Layout::StatsCategoryCount)
                    m_pDraw->DrawArrowAA(panel.right - padX - catArrow, catMidY, catArrow, false, textCol);
            }
        }
        ++row;
    }
    ...
    {
        CStats::ConstructStatLine(i, static_cast<unsigned char>(m_nStatsCategory));
        const std::string label = DecodeGxtScratch(kGxtString);
        const std::string value = DecodeGxtScratch(kGxtString2);
        if (label.empty() && value.empty())
            continue;

        const int rowIdx = row;
        const float y = listTop + static_cast<float>(rowIdx) * rowH - m_fStatsScroll;
        ++row;
        if (y + rowH < listTop)
            continue;
        if (y >= listBottom)
        ...
        const DWORD bg = (rowIdx & 1) ? Layout::PanelZebraB : Layout::PanelZebraA;
        m_pDraw->DrawRect(panel.left, y, pw, rowH, bg);

        if (!label.empty() && value.empty())
        {
            drawPlain(panel.left + padX, y, panel.right - padX, y + rowH,
                      label.c_str(), textCol, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, rowFontH);
        }
        else
        {
            drawPlain(panel.left + padX, y, panel.left + pw * 0.62f, y + rowH,
                      label.c_str(), textCol, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, rowFontH);
            drawPlain(panel.left + pw * 0.38f, y, panel.right - padX, y + rowH,
                      value.c_str(), Layout::StatsValueCol, DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOCLIP, rowFontH);
        }
    }
}

void PauseMenu::HandleStatsInput(float screenW, float screenH)
{
    if (!IsStatsOpen() || m_bExitConfirm)
        return;
    ...
    const float sy = screenH / Layout::RefH;
    const float rowH = Layout::StatsRowH * sy;
    const int notches = (wheel > 0) ? 1 : -1;
    m_fStatsScroll -= static_cast<float>(notches) * rowH * 2.0f;
}

void PauseMenu::DrawLogo(float screenW, float screenH)
{
    if (!m_pDraw || !m_pLogo)
        return;
...
void PauseMenu::DrawButtons(float screenW, float screenH)
{
    if (!m_pDraw)
        return;
...
void PauseMenu::Render()
{
    ...
    if (!IsMapOpen() && !m_bExitConfirm)
    {
        if (IsStatsOpen())
            DrawStatsPanel(w, h);
        else if (m_pMainMenu && m_pMainMenu->IsInitialized())
