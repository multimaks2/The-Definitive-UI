...
float RadarGeometry::ComputeOrbitEdgeFade(float x, float y,
    float centerX, float centerY, float halfX, float halfY,
    bool useSquare, float fadeWidthPx)
{
    if (fadeWidthPx < 1.0f)
        fadeWidthPx = 1.0f;
    ...
    if (useSquare)
    {
        const float ax = (halfX > 0.01f) ? (fabsf(dx) / halfX) : 0.0f;
        const float ay = (halfY > 0.01f) ? (fabsf(dy) / halfY) : 0.0f;
        const float norm = (ax > ay) ? ax : ay;
        const float radius = (halfX < halfY) ? halfX : halfY;
        const float fadeNorm = (radius > 0.01f) ? (fadeWidthPx / radius) : 0.0f;
        if (norm >= 1.0f)
            return 0.0f;
        if (fadeNorm <= 0.0f || norm <= 1.0f - fadeNorm)
            return 1.0f;
        return (1.0f - norm) / fadeNorm;
    }
    ...
    if (dist >= radius)
        return 0.0f;
    if (dist <= radius - fadeWidthPx)
        return 1.0f;
    return (radius - dist) / fadeWidthPx;
}
