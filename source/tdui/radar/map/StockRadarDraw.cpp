...
namespace
{
    ...
    void DrawOrbitSprite(unsigned short spriteId, float x, float y)
    {
        if (!CRadar::RadarBlipSprites)
            return;
        const float size = BlipManager::GetStockSpriteDiameterPx(1.0f);
        const float half = size * 0.5f;
        CRadar::RadarBlipSprites[spriteId].Draw(
            CRect(x - half, y - half, x + half, y + half),
    ...
    void DrawOrbitIndicator(const tRadarTrace& trace, float x, float y, float blipZ, float playerZ)
    {
        ...
        const int scaledSize = static_cast<int>(trace.m_nBlipSize) * RadarConfig::GetBlipIconScalePercent() / 100;
        const unsigned char blipSize = static_cast<unsigned char>((std::min)(255, (std::max)(1, scaledSize)));
        CRadar::ShowRadarTraceWithHeight(
            x, y, blipSize, color.r, color.g, color.b, color.a,
    ...
    void DrawPlayerWaypointOnOrbit()
    {
        ...
        float px = 0.0f;
        float py = 0.0f;
        // Waypoint always above border — Inside disc and Orbit (not Blip RT under rim).
        if (ProjectWorldOntoHud(world.x, world.y, px, py, true) == HudProject::Miss)
            return;
    ...
    void DrawHudOrbitBlips()
    {
        if (!s_plane.use3D || !CRadar::ms_RadarTrace)
            return;
        // Flat 2D: ProjectActivePlane3D already clamps every blip to the disc —
        // a second orbit pass would duplicate sprites (2D-RADAR-GTASA style).
        if (!RadarConfig::GetRadar3D())
            return;
        ...
        for (unsigned int i = 0; i < MAX_RADAR_TRACES; ++i)
        {
            ...
            float px = 0.0f;
            float py = 0.0f;
            const HudProject proj = ProjectWorldOntoHud(world.x, world.y, px, py, true);
            if (proj == HudProject::Miss)
                continue;

            // Legends: HUD for Inside + Orbit (above border). Stock: hide from orbit when CHAR/CAR/CP active.
            if (legend)
            {
                if (hideLegendsFromOrbit && proj == HudProject::Orbit)
                    continue;
                DrawOrbitSprite(spriteId, px, py);
                continue;
            }
            // Indicators: HUD only on Orbit (Inside stays in Blip RT under border).
            if (indicator && proj == HudProject::Orbit)
    ...
    void __cdecl DrawRadarSprite_Hook(unsigned short spriteId, float x, float y, unsigned char alpha)
    {
        ...
        if (s_active && UsesCanvasBlipProjection() && RadarConfig::GetBlipEdgeFade())
        {
            const float fade = RadarGeometry::ComputeOrbitEdgeFade(
                x, y, s_plane.cx, s_plane.cy, s_plane.halfX, s_plane.halfY,
                !s_plane.shapeCircle, MathUtils::ScaleRadarLength(18.0f));
            if (fade <= 0.0f)
                return;
...
void StockRadarDraw::Draw(bool gangOverlay, bool gangInMenu)
{
    ...
    else if (!(UsesCanvasBlipProjection() && RadarConfig::GetRadar3D()))
        RadarOverlayCompat::InvokeHudBlips();

    // Legends + waypoint: HUD above border (Inside+Orbit). Indicators: HUD on Orbit only.
    if (s_plane.use3D)
        DrawHudOrbitBlips();
}
