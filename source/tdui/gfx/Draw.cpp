...
void Draw::DrawChevronSolid(float fCenterX, float fCenterY, float fSize, int dir, DWORD dwColor)
{
    if (!m_bInitialized || !m_pDevice || fSize < 0.5f)
        return;

    float tipX = 0.0f, tipY = 0.0f, backX = 0.0f, backY = 0.0f;
    float wingAX = 0.0f, wingAY = 0.0f, wingBX = 0.0f, wingBY = 0.0f;
    const float t = fSize * 0.22f;

    switch (dir & 3)
    {
    case 0: // left
        tipX = fCenterX - fSize * 0.52f;
        backX = fCenterX + fSize * 0.48f;
        wingAX = backX; wingAY = fCenterY - fSize;
        wingBX = backX; wingBY = fCenterY + fSize;
        break;
    case 1: // right
        tipX = fCenterX + fSize * 0.52f;
        backX = fCenterX - fSize * 0.48f;
        ...
        break;
    case 2: // up
        tipY = fCenterY - fSize * 0.52f;
        backY = fCenterY + fSize * 0.48f;
        wingAX = fCenterX - fSize; wingAY = backY;
        wingBX = fCenterX + fSize; wingBY = backY;
        tipX = fCenterX; backX = fCenterX;
        break;
    default: // down
        tipY = fCenterY + fSize * 0.52f;
        backY = fCenterY - fSize * 0.48f;
    ...
    if (dir <= 1)
    {
        fillArm(tipX, fCenterY, wingAX, wingAY, band);
        fillArm(tipX, fCenterY, wingBX, wingBY, band + 6);
    }
    else
    {
        fillArm(fCenterX, tipY, wingAX, wingAY, band);
        fillArm(fCenterX, tipY, wingBX, wingBY, band + 6);
    }
...
{
    DrawChevronSolid(fCenterX, fCenterY, fSize, pointLeft ? 0 : 1, dwColor);
}

void Draw::DrawArrowAA(float fCenterX, float fCenterY, float fSize, bool pointLeft, DWORD dwColor)
{
    DrawArrowAA(fCenterX, fCenterY, fSize, pointLeft ? 0 : 1, dwColor);
}

void Draw::DrawArrowAA(float fCenterX, float fCenterY, float fSize, int dir, DWORD dwColor)
{
    if (!m_bInitialized || !m_pDevice || fSize < 0.5f)
        return;
    ...
    {
        const int a = static_cast<int>(baseA * alphas[i] + 0.5f);
        DrawChevronSolid(fCenterX, fCenterY, fSize + fringes[i], dir,
                         rgb | (static_cast<DWORD>(a) << 24));
    }
    DrawChevronSolid(fCenterX, fCenterY, fSize, dir, dwColor);
}
