...
namespace
{
    ...
    const char* PrettyKeyName(const char* raw)
    {
        ...
        static const struct { const char* a; const char* b; } kMap[] = {
            { "RETURN", "Enter" }, { "ENTER", "Enter" }, { "ENT", "Enter" },
            { "SPACE", "Пробел" }, { "BSPACE", "Backspace" }, { "BACKSPACE", "Backspace" },
            ...
            { "SPACEBAR", "Пробел" }, { "ZPO6EL", "Пробел" }, { "ПРОБЕЛ", "Пробел" },
            { "MS WHEEL UP", "Колесико вверх" }, { "MS WHEEL DN", "Колесико вниз" },
            { "MS WHEEL DOWN", "Колесико вниз" }, { "MOUSEWHEEL", "Колесико" },
            { "KOLECNKO BVERX", "Колесико вверх" }, { "KOLECNKO BHN3", "Колесико вниз" },
            { "KOLECNKO BBEPX", "Колесико вверх" },
    ...
    const std::string* LookupKey(const char* key)
    {
        ...
        if (lang != LanguageManager::Lang::Russian)
        {
            if (const std::string* s = LookupHash(LanguageManager::Lang::Russian, hash))
                return s;
        }
        return nullptr;
    }

    struct HelpPhraseRow
    {
        const char* key;
        const char* t[kLangCount];
    };

    const HelpPhraseRow kHelpPhrases[] = {
#include "HelpPhrases.inc"
    };

    void IngestPhrases()
    {
        for (const HelpPhraseRow& row : kHelpPhrases)
        {
            if (!row.key || !row.key[0])
                continue;
            const unsigned hash = CKeyGen::GetUppercaseKey(row.key);
            for (int li = 0; li < kLangCount; ++li)
            {
                if (row.t[li] && row.t[li][0])
                    s_maps[li][hash] = row.t[li];
            }
        }
    }
