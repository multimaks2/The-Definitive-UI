                                          ...
                                          const FrustumParams* frustumParams, F&& callback) const
{
    ...
    for (int index = 0; index < MAP_CHUNKS_COUNT; ++index)
    {
        ...
        // Distance around focus (player icon). Camera sits |offsetY| behind that focus;
        // centering on cameraPos left forward tiles on-screen but outside the circle.
        if (MathUtils::DistanceSq2D(chunkCenterX, chunkCenterY, focusPos.x, focusPos.y) > radiusSq)
            continue;

        // Frustum: NDC AABB of the tile must overlap the camera view cone (not
        // merely "any corner in front of the camera" — that kept ~19–21 tiles).
        if (frustumParams && frustumParams->cameraPos && frustumParams->cameraRot)
        {
            if (!MathUtils::IsMapQuadInView(
                    ...
                    *frustumParams->cameraPos, *frustumParams->cameraRot,
                    ...
                    frustumParams->projectionAspect, 0.22f))
                continue;
        }
